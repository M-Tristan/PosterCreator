<template>
  ...
  >
    ...
    <template #trigger>
      <div style="width: 100%; height: 100%" @click="handleTrigger"></div>
    </template>
  </el-popper>
</template>
...
import { useGlobalConfig } from "element-plus/lib/utils/util";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const hue = ref(null as any);
    const svPanel = ref(null as any);
    const alpha = ref(null as any);
    ...
    watch(
      ...
      () => {
        nextTick(() => {
          if (hue.value && hue.value.update) {
            hue.value?.update();
          }
          if (svPanel.value && svPanel.value.update) {
            svPanel.value?.update();
          }
          if (alpha.value && alpha.value.update) {
            alpha.value?.update();
          }
        });
      }
