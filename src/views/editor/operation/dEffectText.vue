<template>
  <div class='effect-text-content' @click='selectModel'  @mousedown="moduleMove(module)" :style="{  width: module.width + 'px'
  ...
  <svg v-if="pattern == 'show'"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" :width="module.width" :height="module.height">
    <path  :d="path" fill="none" stroke-width="1" :id="module.id" ref='pathDom'></path>
    <text :style="{
          ...
          }" :fill='module.color' :stroke='module.strokeColor' :stroke-width='module.strokeWidth' >
  ...
  setup (props) {
    ...
    const path = computed(()=>{
      ...
      switch(module.value.shape){
        case 'circle':
          path = `M ${module.value.fontSize} ${module.value.height/2} A 1 1 0 1 1 ${module.value.width-module.value.fontSize} ${module.value.height/2} M ${module.value.width-module.value.fontSize} ${module.value.height/2} A 1 1 0 1 1 ${module.value.fontSize} ${module.value.height/2}`
          break;
        case 'heart':
          let r = (module.value.width-3*module.value.fontSize)/(2+Math.sqrt(2))
