<template>
  <div class='shape-content'  @click='selectModel'  @mousedown="moduleMove(module)" :style="{  width: module.width + 'px'
...
export default defineComponent({
  ...
  setup (props) {
    ...
    function drawPolygon(){
     ...
     function drawStart(){
        let points = PositionUtil.getStartShapesPoints(module.value.angles,250,150)
        let canvas = shape.value
        let ctx=<CanvasRenderingContext2D>canvas.getContext("2d");
        ctx.clearRect(0,0,500,500);
        ctx.beginPath();
    ...
    function drawSector(){
      ...
      ctx.beginPath();
      ctx.moveTo(250,250);
      ctx.arc(250,250,250,0,module.value.sectorAngle/180*Math.PI,true);
      ctx.closePath();
      ctx.stroke();
      ctx.fillStyle = module.value.color
      ctx.fill();
